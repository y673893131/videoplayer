...
bool BitmapClass::Initialize(ID3D11Device* device, unsigned int screenWidth, unsigned int screenHeight,
...
TextureClass *BitmapClass::texture()
{
    return m_Texture;
}
...
bool BitmapClass::LoadTexture(ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
	bool result;
    ...
    result = m_Texture->Initialize(device, deviceContext, m_screenWidth, m_screenHeight);
	if(!result)
	{
		return false;
	}
