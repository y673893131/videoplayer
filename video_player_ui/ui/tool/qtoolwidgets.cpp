...
void QToolWidgets::CreateMenu(QWidget *parent)
{
    ...
    connect(capture, &QAction::triggered, this, &QToolWidgets::capture);
...
void QToolWidgets::addSoundtrackMenu(QMenu * parent)
{
    auto menuSoundtrack =parent->addMenu(tr("soundtrack"));
    menuSoundtrack->setObjectName("menu1");
    auto both = menuSoundtrack->addAction(tr("both"));
    auto left = menuSoundtrack->addAction(tr("left"));
    auto right = menuSoundtrack->addAction(tr("right"));
    auto group = new QActionGroup(parent);
    group->addAction(both);
    ...
    connect(group, &QActionGroup::triggered, [group, this](QAction *action)
    {
        int sel = group->actions().indexOf(action);
        emit soundTrack(sel);
    });
}
