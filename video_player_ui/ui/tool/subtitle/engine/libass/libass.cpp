...
class LibassPrivate : public VP_Data<Libass>
{
    VP_DECLARE_PUBLIC(Libass)
    inline LibassPrivate(Libass* parent)
        : VP_Data(parent)
        , m_interface(new QLibASSInterface)
        , m_assLib(nullptr)
        , m_assRender(nullptr)
        , m_assTrack(nullptr)
        , m_bFlush(false)
    ...
    ~LibassPrivate()
    {
        if(m_assRender)
        {
            m_interface->ass_renderer_done(m_assRender);
            m_assRender = nullptr;
        }

        if(m_assTrack)
        {
            m_interface->ass_free_track(m_assTrack);
            m_assTrack = nullptr;
        }

        if(m_assLib)
        {
            m_interface->ass_library_done(m_assLib);
            m_assLib = nullptr;
        }
...
#define MSGL_DBG2 7
    ...
    void init()
    {
        ...
        VP_Q(Libass);
        // init ass
        m_assLib = m_interface->ass_library_init();
        m_interface->ass_set_message_cb(m_assLib, callback, q);

        if(!m_assRender)
        {
            m_assRender = m_interface->ass_renderer_init(m_assLib);
        }

        m_interface->ass_set_fonts(m_assRender, nullptr, "Arial",
                                   ASS_FONTPROVIDER_AUTODETECT, nullptr, 1);
    }

private:
    QLibASSInterface* m_interface;
...
Libass::Libass(QObject* parent)
    : QObject(parent)
    , VP_INIT(new LibassPrivate(this))
{
    initAAS();
}
...
void Libass::initTrack(const QByteArray &data)
{
    ...
    if(d->m_assTrack)
    {
        d->m_interface->ass_free_track(d->m_assTrack);
        d->m_assTrack = nullptr;
    }
    d->m_assTrack = d->m_interface->ass_new_track(d->m_assLib);
    d->m_interface->ass_process_codec_private(d->m_assTrack, const_cast<char*>(data.constData()), data.size());
}


void Libass::render(const QString &ass, int pts, int /*dur*/)
{
    VP_D(Libass);
    d->m_start = pts;
    if(d->m_assRender)
    {
        if(d->m_assTrack)
        {
            auto data = ass.toUtf8();
//            ass_process_chunk(d->m_assTrack, data.data(), data.size(), pts, dur);
            d->m_interface->ass_process_data(d->m_assTrack, data.data(), data.size());
        }
...
void Libass::posChange(int pts)
{
    VP_D(Libass);
    int changed = 0;
    auto image = d->m_interface->ass_render_frame(d->m_assRender, d->m_assTrack, pts, &changed);
    if(!image || !changed)
    {
//        qDebug() << "image is null" << changed << pts;
        return;
    }
...
void Libass::setFrameSize(int w, int h)
{
    ...
    if(d->m_assRender)
    {
        d->m_interface->ass_set_frame_size(d->m_assRender, w, h);
...
void Libass::flush()
{
    ...
    {
        d->m_image.fill(Qt::transparent);
        d->m_interface->ass_flush_events(d->m_assTrack);
        d->m_bFlush = true;
    }
}
